...
因此，总的 FLOPs 数量计算如下：
*   对于第一个矩阵乘法：2 X (所有维度的乘积)
*   对于第二个矩阵乘法：2 X (所有维度的乘积)
...
**$H_1$ 的转置** 乘以 **损失对 $H_2$ 的梯度** ![](img/lec2_023.png)，然后通常还需要对批次维度进行求和（sum over the batch dimension）。
