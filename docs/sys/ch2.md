...
bboltdb 将磁盘文件组织成数据页存储，并定义了四种页面类型：
- 叶子节点页
- 分支节点页
- 元信息页
- 空闲列表页
...
![page header](../resources/ch2-001.svg)


| 结构字段 | 含义 | 类型 |
| ---:|:---:|:---  |
| id | 页面的 id |  uint64 |
| flags | 页面的 flags | uint16 |
| count | 页面包含元素的个数 | uint16 |
| overflow | xxx | uint32 |
...
![meta page](../resources/ch2-002.svg)

| 结构字段 | 含义 | 类型 |
| ---:|:---:|:---  |
| magic | 魔数 |  uint32 |
| version | 版本号 | uint32 |
| pageSize | 此 db 页面的大小 | uint32 |
| flags | 页面的标志位  | uint32 |
| root | | |
| freelist | 空闲列表页起始页面 id | uint64 |
| pgid | 页面id | uint64 |
| txid | 事务版本号 | uint64 |
| checksum | 校验和 | uint64 |
...
空闲列表页的组织比较简单，它存储当前空闲页面的 Pgid 列表。

![freelist page](../resources/ch2-003.svg)

#### 3.叶子节点页

![leaf page](../resources/ch2-004.svg)
...
| pgid pgid	 | 当前node在mmap内存中相应的页id，通常一个节点对应一到多个连续的数据页 | 
| parent *node | 父节点指针 | 
...
| ---:|:---:| 
| root() | 返回该树的根节点 |
| minKeys() | 节点至少应有的key的个数，leafNode返回1，branchNode返回2 |
| size()  | 返回节点序列化之后的长度 |
| sizeLessThan()  | 计算节点序列化之后是否小于 v |
| pageElementSize() | 按节点类型返回节点序列化之后的字节数 |
| childAt() | 获取 node 第 index 个孩子节点的指针  |
| childIndex()  | 返回给定子节点在当前节点 inode 中的序号 |
| numChildren()  |  返回当前节点拥有 inode 节点的个数 |
| nextSibling()  | 返回拥有同一个父节点的下一个兄弟节点 |
| prevSibling()  | 返回拥有同一个父节点的上一个兄弟节点 |
| put() | 向节点中插入一个 kv 对，创建新的 inode 节点存储 kv 信息 |
| del() | 删除节点中的 kv 对，删除对应的 inode 节点 |
| read() | 从磁盘页面中读取数据到内存 node 结构 |
| write() | 将内存中的页面数据写到磁盘页面上 |
| split() | 将 node 分裂：如上小结所说，如果 B+ 树节点满了，就需要通过分裂操作来拆分节点 |
| rebalance() | 重新平衡树结构 |

### 四、上层 Bucket 基本 API 实现
